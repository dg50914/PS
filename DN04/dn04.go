...
var stNarocil int
var lock sync.Mutex
var proizvajalecWg sync.WaitGroup
var porabnikWg sync.WaitGroup

func proizvajalec(id int, narocila []narocilo, skladisce chan<- narocilo, stop <-chan any) {
	defer func() {
		if DEBUG {
			fmt.Printf("== Proizvajalec %d exiting\n", id)
		}
		proizvajalecWg.Done()
	}()
	...
	for {
		...
		case <-stop:
			return
		default:
			randomOrder := narocila[rand.Intn(len(narocila))]
			skladisce <- randomOrder
		}
		time.Sleep(time.Millisecond * 500)
	}
}

func porabnik(id int, skladisce <-chan narocilo) {
	defer func() {
		if DEBUG {
			fmt.Printf("== Porabnik %d exiting\n", id)
		}
		porabnikWg.Done()
	}()

	for order := range skladisce {
		order.obdelaj()
		time.Sleep(time.Millisecond * 500)
	}
...
func main() {
	...
	skladisce := make(chan narocilo, 100)
	quit := make(chan any)

	proizvajalecCount := 5

	for i := 0; i < proizvajalecCount; i++ {
		proizvajalecWg.Add(1)
		go proizvajalec(i, narocila, skladisce, quit)
	}

	for i := 0; i < 2; i++ {
		porabnikWg.Add(1)
		go porabnik(i, skladisce)
	}

	readKey(quit, proizvajalecCount)
...
func readKey(input chan any, count int) {
	_, _ = fmt.Scanln()
	if DEBUG {
